"use strict";
  ...
  if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=147',{updateViaCache:'none'}).then(registration=>registration.update()).catch(()=>{}));
  ...
  document.addEventListener('input',event=>{if(event.target.matches('.pin-input'))event.target.setCustomValidity(event.target.value&&/^\d{6}$/.test(event.target.value)?'':'教師画面PINは数字6桁で入力してください。');});
  ...
  document.addEventListener('invalid',event=>{if(event.target.matches('.pin-input'))event.target.setCustomValidity('教師画面PINは数字6桁で入力してください。');},true);
