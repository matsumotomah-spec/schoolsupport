(function(){
  ...
  const HELP_TOPICS={
    home:['教師用ホーム','クラスを選び、今日使う機能を開きます。赤い数字は対応が必要な児童数です。','児童に渡すときは、画面下の「児童用の提出画面」を押してください。'],
    daily:['毎日の宿題','児童名を押すたびに、提出→忘れた→未確認の順で切り替わります。','前回の忘れ物が残る児童は確認画面が開きます。'],
    weekly:['週宿題','宿題を選び、児童名を押して提出状況を切り替えます。','過去週の提出状況は児童の「詳細」からまとめて直せます。'],
    memo:['児童メモ','児童を選び、教科とプラス評価タグを選ぶだけで保存できます。','個別支援級では、今期にメモがない教科を上部に表示します。'],
    certificate:['ミニ賞状','1回目で配付を記録し、2回目で理由タグや取消を選びます。','タグは設定の「タグ」から編集できます。'],
    assessment:['ノート評価','教科・単元を設定し、児童を1回押すとB、もう一度押すと詳しい評価を選べます。','欠席・未提出も評価選択画面から記録できます。'],
    occasional:['提出物','登録済みの提出物を一覧で確認し、児童ごとの提出状況を記録します。','「＋ 新しい提出物を作る」から複数の提出物を追加できます。'],
    seating:['席替え','条件を設定して席を作り、警告を確認しながら調整します。','ドラッグ後も、満たせていない配慮条件が再計算されます。'],
    settings:['設定','最初に設定項目の説明を確認し、必要な項目を選びます。','クラス・名簿、タグ、年度、データ管理をここにまとめています。'],
    data:['データ管理','端末間同期、バックアップ、CSV出力、旧データ移行を行います。','同期は日常の記録統合、バックアップは故障や年度保管への備えです。'],
    support:['単元設定','教科ごとの現在の学習単元を作り、児童をドラッグしてまとめます。','児童を選ぶと、その教科・単元の学習記録を入力できます。'],
  ...
  function renderSetup(){
    ...
    app.innerHTML=`
      ...
      <main class="page narrow">
          <section class="welcome-card"><h1>ようこそ</h1><p>クラス作成と名簿登録まで順に案内します。旧ツールのデータがある場合は、先に確認できます。</p><button type="button" class="button" id="setup-legacy-check">旧データを確認</button></section>
          ...
          <div class="field"><label for="setup-mode">自分のクラス</label><select class="select" id="setup-mode"><option value="general">一般級</option><option value="support">個別支援級</option></select></div>
            ...
            <div class="field" id="setup-support-name" hidden><label for="setup-class">個別支援級の名前</label><input class="input" id="setup-class" value="ひまわり" placeholder="例：ひまわり"></div>
  ...
  function headerHtml(subtitle,actions='',homeActive=false,teacherControls=true){const common=teacherControls?`<button type="button" class="header-button home-button ${homeActive?'active':''}" data-common-home aria-current="${homeActive?'page':'false'}" title="教師用ホームへ戻る"><span aria-hidden="true">⌂</span> ホーム</button><button type="button" class="header-button" data-common-settings title="設定項目の説明を開く">⚙ 設定</button><button type="button" class="header-button header-icon" data-context-help aria-label="この画面の使い方" title="この画面の使い方">?</button>`:'';return `<header class="app-header"><div class="brand-block"><div class="app-title">クラス支援ツール</div><div class="app-subtitle">${esc(state.year?.label||'')}　${esc(subtitle)}</div>${connectionStatusHtml()}</div><div class="header-actions">${common}${actions}${teacherControls?`<button type="button" class="header-button header-icon theme-toggle" data-theme-toggle aria-label="${state.theme==='dark'?'ライトモードに切り替え':'ダークモードに切り替え'}" title="${state.theme==='dark'?'ライトモードに切り替え':'ダークモードに切り替え'}">${state.theme==='dark'?'☀':'☾'}</button>`:''}</div></header>`;}
  ...
  function wireCommonHeader(helpKey='home'){
    document.querySelector('[data-common-home]')?.addEventListener('click',renderHome);
    document.querySelector('[data-common-settings]')?.addEventListener('click',()=>{state.settingsTab='guide';state.classSettingsView='list';renderSettings();});
    document.querySelector('[data-context-help]')?.addEventListener('click',()=>openContextHelp(helpKey));
    applyTheme();
  }
  function openContextHelp(key){const topic=HELP_TOPICS[key]||HELP_TOPICS.home;document.getElementById('context-help-drawer')?.remove();const drawer=document.createElement('aside');drawer.id='context-help-drawer';drawer.className='help-drawer';drawer.setAttribute('aria-label','この画面の使い方');drawer.innerHTML=`<div class="help-drawer-head"><h2>${esc(topic[0])}</h2><button type="button" class="header-button header-icon" data-help-close aria-label="閉じる">×</button></div><h3>できること</h3><p>${esc(topic[1])}</p><h3>使い方</h3><p>${esc(topic[2])}</p><div class="button-row section"><button type="button" class="button" data-full-help>ヘルプ・FAQで探す</button></div>`;document.body.appendChild(drawer);requestAnimationFrame(()=>drawer.classList.add('open'));drawer.querySelector('[data-help-close]').addEventListener('click',()=>drawer.remove());drawer.querySelector('[data-full-help]').addEventListener('click',()=>{drawer.remove();renderHelp();});}
  ...
  async function dashboardHtml(counts){const history=await ClassDB.getMeta('syncHistory',[]),last=history[0],lastLabel=last?`${last.kindLabel||last.kind}・${new Date(last.at).toLocaleString('ja-JP',{month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit'})}`:'まだありません';const items=[['daily','毎日の宿題',counts.daily],['weekly','週宿題',counts.weekly],['occasional','提出物',counts.occasional],['memo','児童メモ',counts.memo]].filter(item=>item[2]>0);return`<section class="dashboard panel"><div class="dashboard-heading"><div><h2>今日の確認</h2><p class="muted">対応が必要なものをまとめています。</p></div><div class="dashboard-sync"><span class="sync-dot ${last?'synced':''}"></span><span>最終同期：${esc(lastLabel)}</span></div></div><div class="dashboard-cards">${items.length?items.map(([id,label,count])=>`<button type="button" class="dashboard-card" data-tool="${id}" title="${esc(label)}を開く"><strong>${count}</strong><span>${label}</span></button>`).join(''):'<div class="dashboard-empty">現在、対応が必要な項目はありません。</div>'}</div></section>`;}
  ...
  function teacherToolShell(title,body,actions=''){
    ...
    return `<div class="app-shell">${headerHtml(title,actions)}<main class="page"><div class="breadcrumb"><button type="button" data-breadcrumb-home>ホーム</button><span>›</span><strong>${esc(title)}</strong></div>${body}</main>${teacherFooter(key)}</div>`;
  }

  function teacherFooter(active){const classItem=selectedClass(),allowed=!classItem?.isOwn&&!isSupportClass(classItem)?['memo','assessment','occasional']:['daily','weekly','certificate','memo','assessment','occasional'];const links={daily:['✓','毎日の宿題'],weekly:['▣','週宿題'],certificate:['☆','ミニ賞状'],memo:['✎','児童メモ'],assessment:['A','ノート評価'],occasional:['▤','提出物']};return`<nav class="teacher-footer" aria-label="日常機能">${allowed.map(id=>`<button type="button" data-footer-tool="${id}" aria-current="${active===id?'page':'false'}" title="${links[id][1]}へ切り替える"><span>${links[id][0]}</span>${links[id][1]}</button>`).join('')}</nav>`;}
  function wireToolHome(){const key=state.activeTool||state.route.replace('teacher-','');wireCommonHeader(key);document.querySelector('[data-breadcrumb-home]')?.addEventListener('click',renderHome);document.querySelectorAll('[data-footer-tool]').forEach(button=>button.addEventListener('click',()=>openTool(button.dataset.footerTool)));}
  ...
  async function renderPupil(tool=state.pupilTool||'daily'){
    ...
    const nav=`<nav class="pupil-nav" aria-label="提出画面"><button type="button" data-pupil-tool="daily" aria-selected="${tool==='daily'}">毎日の宿題</button><button type="button" data-pupil-tool="weekly" aria-selected="${tool==='weekly'}">週宿題</button><button type="button" data-pupil-tool="occasional" aria-selected="${tool==='occasional'}">提出物</button></nav>`;
    app.innerHTML=`<div class="app-shell">${headerHtml(`${esc(classItem?.name||'')}　児童用提出画面`,`<button type="button" class="header-button header-icon" id="teacher-entry" aria-label="教師用設定" title="教師用画面を開く">⚙</button>`,false,false)}<main class="page">${nav}<div id="pupil-content"></div></main></div>`;
  ...
  async function renderSettings(){
    ...
    app.innerHTML=`<div class="app-shell">${headerHtml('設定')}<main class="page">
      <div class="breadcrumb"><button type="button" data-breadcrumb-home>ホーム</button><span>›</span><strong>設定</strong></div>
      <nav class="settings-nav" aria-label="設定項目">${[['guide','設定案内'],['year','年度・認証'],['classes','クラス・名簿'],['tags','タグ'],['prompt','所見'],['data','データ管理'],['help','ヘルプ・FAQ']].map(([id,label])=>`<button type="button" class="settings-tab" data-settings-tab="${id}" aria-selected="${state.settingsTab===id}" title="${label}を開く">${label}</button>`).join('')}</nav>
      <div id="settings-content"></div>
    </main></div>`;
    wireCommonHeader('settings');document.querySelector('[data-breadcrumb-home]').addEventListener('click',renderHome);
    document.querySelectorAll('[data-settings-tab]').forEach(button=>button.addEventListener('click',()=>{state.settingsTab=button.dataset.settingsTab;if(state.settingsTab==='classes'){state.classSettingsView='list';state.rosterDraft=[];state.rosterLoadedForClassId=null;}renderSettingsContent();}));
  ...
  function renderSettingsGuide(){const target=document.getElementById('settings-content');const cards=[['year','年度・認証','年度の日付、教師用PIN、暗号化用パスワードを管理します。'],['classes','クラス・名簿','クラスの追加、教科、色、名簿を設定します。'],['tags','タグ','児童メモ・ミニ賞状・支援級記録の選択肢を編集します。'],['prompt','所見','記録から所見素材とAI用プロンプトを作る設定です。'],['data','データ管理','同期、バックアップ、CSV出力、旧データ移行、削除をまとめています。'],['help','ヘルプ・FAQ','操作方法を検索して確認します。']];target.innerHTML=`<section class="panel"><h1>何を設定しますか</h1><p class="muted">目的を選ぶと、詳しい設定ページへ進みます。</p></section><section class="settings-guide">${cards.map(([id,title,description])=>`<button type="button" class="settings-guide-card" data-guide-tab="${id}" title="${esc(title)}を設定する"><strong>${esc(title)}</strong><span>${esc(description)}</span><b>開く ›</b></button>`).join('')}</section>`;target.querySelectorAll('[data-guide-tab]').forEach(button=>button.addEventListener('click',()=>{state.settingsTab=button.dataset.guideTab;renderSettingsContent();}));}
  ...
  function helpSearchHtml(){return`<div class="field help-search"><label for="help-search">ヘルプ内を検索</label><input class="input" id="help-search" type="search" placeholder="例：名簿、同期、提出物"></div>`;}
  function modernHelpContentHtml(){const cards=[['1','最初にすること','設定からクラス・名簿を開きます。一般級は学年と組、個別支援級はクラス名を設定し、最初から開いている児童欄へ入力します。'],['2','毎日の使い方','教師ホームまたは画面下の機能ナビから、宿題・メモ・評価・提出物を切り替えます。児童へ渡すときだけ「児童用の提出画面」を開きます。'],['3','データを管理する','設定の「データ管理」に、iPad・PC間の同期、暗号化バックアップ、CSV出力、旧データ移行をまとめています。'],['4','表示の見方','赤い数字は対応が必要な児童数です。クラス色は現在操作中のクラスを示します。']];return`<section class="help-grid">${cards.map(([n,title,text])=>`<article class="panel help-card"><span class="help-number">${n}</span><h2>${title}</h2><p>${text}</p></article>`).join('')}</section><section class="panel faq-panel"><h2>よくある質問</h2><details><summary>名簿がない状態で機能を開いたら？</summary><p>そのクラスの名簿設定へ自動的に移動します。保存後に機能を利用できます。</p></details><details><summary>同期とバックアップの違いは？</summary><p>同期はiPadとPCの記録を統合するため、バックアップは故障や年度保管に備えるために使います。</p></details><details><summary>年度パスワードを忘れたら？</summary><p>同じ端末にデータが残っていれば復旧コードだけで再設定できます。新しい端末では暗号化バックアップも必要です。</p></details><details><summary>提出物を追加するには？</summary><p>提出物画面の「＋ 新しい提出物を作る」から複数登録できます。カードを選ぶと児童ごとの状況が開きます。</p></details><details><summary>通信が切れても使えますか？</summary><p>一度読み込んだアプリはオフラインでも使え、記録は端末内に保存されます。</p></details></section><section class="panel help-contact"><h2>困ったときの確認順</h2><p>①現在のクラス　②名簿　③教師用／児童用画面　④年度　⑤同期ファイルの作成日時、の順に確認してください。</p></section>`;}
  function wireHelpSearch(){document.getElementById('help-search')?.addEventListener('input',event=>{const query=event.target.value.trim().toLowerCase();document.querySelectorAll('.help-card,.faq-panel details,.help-contact').forEach(item=>item.hidden=Boolean(query)&&!item.textContent.toLowerCase().includes(query));});}
  function renderHelp(){if(!teacherActive()){renderPupil();return;}state.route='teacher-help';state.activeTool='help';applyClassTheme(selectedClass());app.innerHTML=teacherToolShell('使い方・FAQ',`<section class="panel"><h1>クラス支援ツールの使い方</h1><p class="muted">知りたい言葉で検索できます。</p>${helpSearchHtml()}</section>${modernHelpContentHtml()}`);wireToolHome();wireHelpSearch();}
  function renderHelpContent(){const target=document.getElementById('settings-content');target.innerHTML=`<section class="panel"><h1>使い方・FAQ</h1><p class="muted">知りたい言葉で検索できます。</p>${helpSearchHtml()}</section>${modernHelpContentHtml()}`;wireHelpSearch();}
  ...
  function openClassEditor(item=null){
    const current=item||{name:'',gradeLevel:5,color:COLORS[2],isOwn:false,isSupport:false,recordSubjects:[...SUBJECTS]};const subjects=classSubjects(current);const support=isSupportClass(current),nameMatch=String(current.name||'').match(/^(\d)年(.+)組$/),grade=current.gradeLevel||Number(nameMatch?.[1])||5,group=nameMatch?.[2]||'';
    openDialog(`<h2>${item?'クラスを編集':'新しいクラスを作成'}</h2><form id="class-form"><div class="field"><label for="class-type">クラスの種類</label><select class="select" id="class-type"><option value="general" ${!support?'selected':''}>一般級</option><option value="support" ${support?'selected':''}>個別支援級</option></select></div><div class="field section" id="class-general-name"><label>一般級の学年・組</label><div class="inline-class-name"><select class="select" id="class-grade">${[1,2,3,4,5,6].map(n=>`<option value="${n}" ${n===grade?'selected':''}>${n}年</option>`).join('')}</select><input class="input" id="class-group" value="${esc(group)}" aria-label="組"><span>組</span></div></div><div class="field section" id="class-support-name"><label for="class-name">個別支援級の名前</label><input class="input" id="class-name" value="${esc(support?current.name:'')}" placeholder="例：ひまわり"></div><div class="field section"><span class="field-label">クラスカラー</span><div class="color-choices" id="class-colors">${colorButtons(current.color)}</div></div><label class="check-row"><input id="class-own" type="checkbox" ${current.isOwn?'checked':''}> 自分のクラスにする</label><label class="check-row"><input id="class-support" type="checkbox" ${support?'checked':''}> 個別支援級にする</label><div class="field section"><span class="field-label">このクラスで記録する教科</span><div class="subject-checks">${SUBJECTS.map(subject=>`<label><input type="checkbox" data-class-subject="${subject}" ${subjects.includes(subject)?'checked':''}> ${subject}</label>`).join('')}</div></div><p class="error" id="class-error"></p><div class="dialog-actions"><button type="button" class="button" id="class-cancel">キャンセル</button><button type="submit" class="button primary">保存して名簿へ</button></div></form>`);
    wireColorChoices(document.getElementById('class-colors'));
    const syncType=source=>{if(source==='select')document.getElementById('class-support').checked=document.getElementById('class-type').value==='support';else document.getElementById('class-type').value=document.getElementById('class-support').checked?'support':'general';const isSupport=document.getElementById('class-type').value==='support';document.getElementById('class-general-name').hidden=isSupport;document.getElementById('class-support-name').hidden=!isSupport;};document.getElementById('class-type').addEventListener('change',()=>syncType('select'));document.getElementById('class-support').addEventListener('change',()=>syncType('check'));syncType('select');
    document.getElementById('class-cancel').addEventListener('click',closeDialog);
    document.getElementById('class-form').addEventListener('submit',async event=>{event.preventDefault();const isOwn=document.getElementById('class-own').checked;const isSupport=document.getElementById('class-type').value==='support';const gradeLevel=isSupport?null:Number(document.getElementById('class-grade').value),group=document.getElementById('class-group').value.trim(),custom=document.getElementById('class-name').value.trim(),name=isSupport?custom:`${gradeLevel}年${group}組`;const recordSubjects=[...document.querySelectorAll('[data-class-subject]:checked')].map(input=>input.dataset.classSubject);const color=document.querySelector('#class-colors [data-color-code]').value.trim();if(!name||(!isSupport&&!group)){document.getElementById('class-error').textContent='クラス名を入力してください。';return;}if(!validColor(color)){document.getElementById('class-error').textContent='カラーコードを確認してください。';return;}if(!recordSubjects.length){document.getElementById('class-error').textContent='記録する教科を1つ以上選んでください。';return;}if(isOwn){for(const other of state.classes.filter(x=>x.isOwn&&x.id!==item?.id))await ClassDB.put('classes',{...other,isOwn:false});}const saved=await ClassDB.put('classes',{...(item||{}),id:item?.id||ClassDB.uid('class'),yearId:state.year.id,name,gradeLevel,color,isOwn,isSupport,recordSubjects,order:item?.order??state.classes.length});state.classes=(await ClassDB.getAllByIndex('classes','yearId',state.year.id)).sort((a,b)=>(b.isOwn-a.isOwn)||(a.order-b.order));state.selectedClassId=saved.id;await ClassDB.setMeta('selectedClassId',saved.id);closeDialog();state.classSettingsView='roster';state.rosterDraft=[];state.rosterLoadedForClassId=null;renderClassSettings();showToast('続いて名簿を登録してください');});
  }
  ...
  async function renderRosterSettings(){
    ...
    if(state.rosterLoadedForClassId!==classItem.id){const rows=await rosterForClass(classItem.id);state.rosterDraft=rows.map(({enrollment,student})=>({enrollmentId:enrollment.id,studentId:student.id,number:enrollment.number,name:student.name,grade:enrollment.grade||classItem.gradeLevel||'',gender:enrollment.gender||''}));if(!state.rosterDraft.length)state.rosterDraft=[{number:1,name:'',grade:classItem.gradeLevel||'',gender:''}];state.rosterLoadedForClassId=classItem.id;}
    if(state.settingsTab!=='classes'||state.classSettingsView!=='roster'||selectedClass()?.id!==classItem.id)return;
    target.innerHTML=`<section class="panel"><div class="button-row" style="justify-content:space-between"><div><h1>${esc(classItem.name)}の名簿</h1><p class="muted">まずは下の「手動で1人ずつ登録」から入力できます。Excel・CSV・表の貼り付けにも対応します。</p></div><button type="button" class="button" id="roster-back">クラス一覧へ</button></div></section><section class="panel manual-roster"><h2>手動で1人ずつ登録</h2><p class="muted">1人目の入力欄を開いてあります。一般級はクラスの学年を自動入力し、必要なら児童ごとに変更できます。</p><div class="table-wrap"><table class="roster-table"><thead><tr><th>番号</th><th>氏名</th><th>学年</th><th>性別</th><th></th></tr></thead><tbody id="roster-body">${rosterRowsHtml()}</tbody></table></div><div class="button-row"><button type="button" class="button" id="blank-row">＋ 次の児童を追加</button></div></section><details class="panel roster-import"><summary>Excel・CSV・貼り付けからまとめて登録</summary><div class="form-grid section"><div class="field full"><label for="roster-paste">表を貼り付け</label><textarea class="textarea" id="roster-paste" placeholder="1\t青木 陽斗\t5\t男"></textarea></div><div class="field"><label for="roster-file">Excel・CSV・テキストファイル</label><input class="input" id="roster-file" type="file" accept=".xlsx,.csv,.txt,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,text/csv,text/plain"></div><div class="field" style="align-self:end"><button type="button" class="button" id="parse-roster">貼り付け内容を追加</button></div></div></details><section class="panel"><p class="error" id="roster-error" role="alert"></p><div class="button-row end"><button type="button" class="button primary" id="save-roster">名簿を保存</button></div></section>`;
    wireRosterRows();
    ...
    document.getElementById('blank-row').addEventListener('click',()=>{readRosterInputs();state.rosterDraft.push({number:state.rosterDraft.length+1,name:'',grade:classItem.gradeLevel||'',gender:''});renderRosterSettings();});
  ...
  async function saveRoster(){
    ...
    if(valid.some(row=>!String(row.grade||'').trim())){document.getElementById('roster-error').textContent='学年が未設定の児童がいます。1～6年から選んでください。';return;}
    const classItem=selectedClass();
  ...
  async function renderMemos(){
    ...
    const cards=await teacherRosterCards(classItem.id,[...latest.values()],{orderMode:teacherOrderMode(),status:record=>record?(record.tags?.join('・')||record.viewpoint||'記録あり'):'記録を追加'});
  ...
  async function renderCertificates(){
    ...
    const cards=await teacherRosterCards(classItem.id,records,{orderMode:teacherOrderMode(),status:record=>record?(record.tags?.length?record.tags.join('・'):'渡した'):'未配付',statusClass:record=>record?'good':''});
    app.innerHTML=teacherToolShell('ミニ賞状',`${pupilDateNav(date,'certificate-prev','certificate-next')}<section class="panel"><div class="toolbar-line"><div><h1>${esc(classItem.name)}のミニ賞状</h1><p class="muted">1回目で「渡した」。もう一度選ぶと理由メモの追加や取消ができます。</p></div>${teacherOrderControlHtml()}</div></section>${cards}`);
  ...
  async function renderOccasional(){
    ...
    app.innerHTML=teacherToolShell('提出物',`<section class="panel"><div class="toolbar-line"><div><h1>${esc(classItem.name)}の提出物</h1><p class="muted">複数の提出物を登録し、カードを選んで状況を確認できます。</p></div><button type="button" class="button primary" id="occasional-create">＋ 新しい提出物を作る</button></div><div class="submission-item-list section">${itemCards||'<div class="empty-state"><h2>提出物はまだありません</h2><p>右上の「＋ 新しい提出物を作る」から登録してください。</p></div>'}</div></section>${item?`<section class="panel"><div class="toolbar-line"><div><h2>${esc(item.title)}</h2><p class="muted">提出予定 ${esc(jpDate(item.dueDate))}・${item.archived?'完結済み':'対応中'}</p></div><div class="button-row"><button type="button" class="button" id="occasional-edit">編集</button><button type="button" class="button" id="occasional-archive">${item.archived?'再開する':'完結・アーカイブ'}</button>${teacherOrderControlHtml()}</div></div></section>${cards}`:''}`);
  ...
  async function renderStudentOverview(studentId,tab='summary'){
    ...
    const addLabels={memo:'＋ メモを追加',assessment:'＋ ノート評価を追加',certificate:'＋ ミニ賞状を追加',homework:'＋ 提出物を記録'};const unresolvedHtml=daily.map(item=>unresolvedRowHtml('daily',item,'毎日の宿題',item.date)).join('')+weeklyMissing.map(item=>unresolvedRowHtml('weekly',item,item.title,item.dueDate)).join('')+occasionalMissing.map(item=>unresolvedRowHtml('occasional',item,item.title,item.dueDate)).join('');
    let content='';
    ...
    document.querySelectorAll('[data-resolve-daily]').forEach(button=>button.addEventListener('click',async()=>{const record=await ClassDB.get('records',button.dataset.resolveDaily);if(record)await resolvePrior(record);showToast('宿題を解決済みにしました');renderStudentOverview(studentId,'homework');}));
    document.querySelectorAll('[data-resolve-weekly]').forEach(button=>button.addEventListener('click',async()=>{const occurrence=weekly.occurrences.find(item=>item.id===button.dataset.resolveWeekly);if(occurrence){const id=`weekly_${occurrence.id}_${studentId}`;const current=await ClassDB.get('records',id);await ClassDB.put('records',{...(current||{}),id,type:'weeklySubmission',classId:classItem.id,studentId,date:occurrence.dueDate,dueDate:occurrence.dueDate,title:occurrence.title,occurrenceId:occurrence.id,status:'submitted'});}showToast('週宿題を提出済みにしました');renderStudentOverview(studentId,'homework');}));
    document.querySelectorAll('[data-resolve-occasional]').forEach(button=>button.addEventListener('click',async()=>{const item=occasional.items.find(row=>row.id===button.dataset.resolveOccasional);if(item){const id=`occasional_${item.id}_${studentId}`,current=await ClassDB.get('records',id);await ClassDB.put('records',{...(current||{}),id,type:'occasionalSubmission',classId:classItem.id,studentId,date:today(),dueDate:item.dueDate,title:item.title,itemId:item.id,status:'submitted'});}showToast('提出済みにしました');renderStudentOverview(studentId,'homework');}));
  }

  function unresolvedRowHtml(kind,item,title,date){const labels={daily:'忘れ物を解決',weekly:'提出済みにする',occasional:'提出済みにする'},attrs={daily:`data-resolve-daily="${item.id}"`,weekly:`data-resolve-weekly="${item.id}"`,occasional:`data-resolve-occasional="${item.id}"`};return`<div class="unresolved-row"><div><strong>${esc(title)}</strong><span>${esc(jpDate(date))}・${kind==='daily'?'忘れたまま':'未提出'}</span></div><button type="button" class="button primary" ${attrs[kind]}>${labels[kind]}</button></div>`;}
  ...
  async function openStudentAssessmentAdd(studentId,returnTab){const student=await ClassDB.get('students',studentId),classItem=selectedClass(),subjects=classSubjects(classItem);openDialog(`<h2>${esc(student.name)}のノート評価を追加</h2><form id="quick-assessment-form"><div class="form-grid"><div class="field"><label for="quick-assessment-date">日付</label><input class="input" id="quick-assessment-date" type="date" value="${today()}" required></div><div class="field"><label for="quick-assessment-subject">教科</label><select class="select" id="quick-assessment-subject">${subjects.map(subject=>`<option>${esc(subject)}</option>`).join('')}</select></div><div class="field"><label for="quick-assessment-unit">単元</label><input class="input" id="quick-assessment-unit"></div><div class="field"><label for="quick-assessment-title">題名（任意）</label><input class="input" id="quick-assessment-title"></div><div class="field full"><label for="quick-assessment-grade">評価</label><select class="select" id="quick-assessment-grade"><option>A</option><option>B+</option><option selected>B</option><option>B-</option><option>C</option><option value="absent">欠席</option><option value="unsubmitted">未提出</option></select></div></div><div class="dialog-actions"><button type="button" class="button" data-quick-cancel>キャンセル</button><button type="submit" class="button primary">保存</button></div></form>`);wireQuickCancel();document.getElementById('quick-assessment-form').addEventListener('submit',async event=>{event.preventDefault();const draft={date:document.getElementById('quick-assessment-date').value,subject:document.getElementById('quick-assessment-subject').value,unit:document.getElementById('quick-assessment-unit').value.trim(),title:document.getElementById('quick-assessment-title').value.trim()},value=document.getElementById('quick-assessment-grade').value,sessionKey=notebookSessionKey(draft);await ClassDB.put('records',{id:`notebook_${classItem.id}_${sessionKey}_${studentId}`,type:'notebookAssessment',classId:classItem.id,studentId,date:draft.date,subject:draft.subject,unit:draft.unit,title:draft.title||slashDate(draft.date),sessionKey,grade:['absent','unsubmitted'].includes(value)?null:value,status:['absent','unsubmitted'].includes(value)?value:'evaluated'});closeDialog();showToast('ノート評価を追加しました');renderStudentOverview(studentId,returnTab);});}
  ...
  async function renderDataExchange(inSettings=false){
    if(!teacherActive()){renderPupil();return;}
    state.route='teacher-data';state.activeTool='data';state.dataInSettings=inSettings;applyClassTheme(selectedClass());
    ...
    const body=`
      ...
      <section class="panel danger-zone"><h1>クラスとデータの削除</h1><p class="muted">旧ツールのデータは削除しません。対象件数を確認してから実行します。</p><div class="button-row"><button type="button" class="button danger" id="delete-class-records">現在のクラスの記録を削除</button><button type="button" class="button danger" id="delete-current-class">名簿を含めてクラスを削除</button></div></section>`;
  ...
  function renderDataSettings(){document.getElementById('settings-content').innerHTML=`<section class="panel"><h1>データ管理</h1><p class="muted">同期と出力・移行を一つにまとめました。端末間で記録を合わせるとき、バックアップを残すとき、PCへ出力するときに使います。</p><div class="credential-guide section"><div><strong>端末間同期</strong><span>iPadとPCの更新日時を比較して統合</span></div><div><strong>バックアップ</strong><span>故障や年度切替に備えて暗号化保存</span></div><div><strong>出力・移行</strong><span>CSV出力と旧ツールからの移行</span></div></div><div class="button-row end section"><button class="button primary" id="open-data-exchange">データ管理を開く</button></div></section>`;document.getElementById('open-data-exchange').addEventListener('click',()=>renderDataExchange(true));}
