"use strict";
  ...
  if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=129',{updateViaCache:'none'}).then(registration=>registration.update()).catch(()=>{}));
  ...
  document.addEventListener('input',event=>{const roster=event.target.closest('#roster-body'),form=event.target.closest('#app form');if(roster)markRosterDraftDirty();if(form&&!form.matches('[data-auto-save]'))markFormDraftDirty(form);});
  document.addEventListener('submit',event=>{clearFormDraft(event.target);},true);
