"use strict";
    ...
    app.innerHTML=`<div class="app-shell">
      ...
      <main class="page">
        ...
        <section class="tools-main ${!own&&!support?'limited':''}">
          ${!own&&!support?`${toolHtml('memo','✎','児童メモ',counts.memo,true)}${toolHtml('assessment','A','ノート評価',0)}${toolHtml('tests','テ','小テスト',0)}${toolHtml('grades','点','成績管理',0)}${toolHtml('occasional','▤','提出物',counts.occasional)}`:`
  ...
  function teacherFooter(active){const classItem=selectedClass(),allowed=!classItem?.isOwn&&!isSupportClass(classItem)?['memo','assessment','tests','grades','occasional']:['daily','weekly','certificate','memo','assessment','tests','grades','occasional'];const labels={daily:'毎日の宿題',weekly:'週宿題',certificate:'ミニ賞状',memo:'児童メモ',assessment:'ノート評価',tests:'小テスト',grades:'成績管理',occasional:'提出物'};return`<nav class="teacher-footer" aria-label="日常機能" style="--footer-count:${allowed.length}">${allowed.map(id=>`<button type="button" data-footer-tool="${id}" aria-current="${active===id?'page':'false'}" title="${labels[id]}へ切り替える"><span>${featureIcon(id)}</span>${labels[id]}</button>`).join('')}</nav>`;}
  ...
  async function renderPupilDaily(){
    ...
    document.getElementById('pupil-content').innerHTML=`<div class="pupil-current-date">${esc(pupilDateText(date))}</div><div class="status-legend"><span class="legend-submitted">${esc(pupilStatusLabel('submitted'))}</span><span class="legend-forgotten">${esc(pupilStatusLabel('forgotten'))}</span><span class="legend-absent">${esc(pupilStatusLabel('absent'))}</span><span class="legend-unconfirmed">${esc(pupilStatusLabel('unconfirmed'))}</span></div><div class="summary-row"><span>${esc(pupilText('提出','だした'))} <strong>${totals.submitted}</strong></span><span>${esc(pupilText('忘れた','わすれた'))} <strong>${totals.forgotten}</strong></span><span>${esc(pupilText('欠席','おやすみ'))} <strong>${totals.absent}</strong></span><span>${esc(pupilText('未確認','まだ'))} <strong>${totals.unconfirmed}</strong></span></div>${roster.length?pupilStudentGrid(classItem,roster,map,'daily',priorForgotten,visibleRewardMedals(medalData)):`<section class="panel"><h2>${esc(pupilText('名簿が未登録です','なまえが とうろく されていません'))}</h2><p class="muted">${esc(pupilText('右上の歯車から教師認証し、名簿を登録してください。','せんせいに しらせてね。'))}</p></section>`}`;
  ...
  function studentCard(row,record,kind='daily',priorForgotten=false,medals=new Set()){
    const defaults={daily:'unconfirmed',weekly:'unsubmitted',occasional:'unsubmitted'};const status=record?.status||defaults[kind],exchangeSeatOnly=submissionExempt(row);
    const number=row.enrollment?.number?`${row.enrollment.number}. `:'';
    return `<button type="button" class="student-card ${exchangeSeatOnly?'exchange-seat-only':status}${!exchangeSeatOnly&&priorForgotten?' has-prior-forgotten':''}${feedbackClass(row.student.id)}" data-student-id="${row.student.id}" ${exchangeSeatOnly?'disabled':''}><strong>${esc(number)}${esc(row.student.name)}${!exchangeSeatOnly&&medals.has(row.student.id)?rewardIconHtml('medal-inline'):''}</strong>${exchangeSeatOnly?'':`<span class="state ${status}">${esc(pupilStatusLabel(status))}</span>${priorForgotten?`<span class="prior-flag">${esc(pupilText('今週の忘れあり','こんしゅう わすれた'))}</span>`:''}`}</button>`;
  }
