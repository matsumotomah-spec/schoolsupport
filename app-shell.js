"use strict";
  ...
  async function attentionCounts(classId){
    ...
    const priorForgotten=new Set(records.filter(record=>record.type==='dailyHomework'&&record.date>=currentWeekStart()&&record.date<today()&&record.status==='forgotten'&&!record.resolvedAt&&!record.deletedAt&&activeStudentIds.includes(record.studentId)).map(record=>record.studentId));
  ...
  async function renderTeacherDaily(){
    ...
    const totals={submitted:0,forgotten:0,unconfirmed:0,absent:0};const map=new Map(records.map(record=>[record.studentId,record]));roster.filter(row=>!submissionExempt(row)).forEach(row=>{const status=map.get(row.student.id)?.status||'unconfirmed';totals[status]=(totals[status]||0)+1;});
    const labels={unconfirmed:'未確認',submitted:'提出',forgotten:'忘れた',absent:'欠席'};const cards=await teacherRosterCards(classItem.id,records,{orderMode:teacherOrderMode(),preserveSeatShape:true,atDate:date,status:(record,row)=>submissionExempt(row)?'提出管理の対象外':`${labels[record?.status||'unconfirmed']}${priorForgotten.has(row.student.id)?'・今週の忘れあり':''}`,statusClass:(record,row)=>submissionExempt(row)?'muted':record?.status==='forgotten'||priorForgotten.has(row.student.id)?'alert':record?.status==='submitted'?'good':record?.status==='absent'?'absent':'',extra:(record,row)=>!submissionExempt(row)&&medalData.medals.has(row.student.id)?rewardIconHtml():''});
    app.innerHTML=teacherToolShell('毎日の宿題',`${pupilDateNav(date,'teacher-daily-prev','teacher-daily-next')}<section class="panel daily-guide"><div class="toolbar-line"><div><h1>今日の提出確認</h1><p class="muted">児童名を押して記録します。未解決の忘れ物は今週分だけ表示します。</p></div>${teacherOrderControlHtml()}</div>${operationTipHtml('押し方を見る','児童名は、提出 → 忘れた → 未確認の順で切り替わります。欠席は右の「欠席を設定」から選びます。')}<div class="summary-row"><span>提出 <strong>${totals.submitted}</strong></span><span>忘れた <strong>${totals.forgotten}</strong></span><span>欠席 <strong>${totals.absent}</strong></span><span>未確認 <strong>${totals.unconfirmed}</strong></span></div><div class="button-row section"><button type="button" class="button" id="daily-absence-setting">欠席を設定</button></div><div class="medal-setting"><label for="homework-medal-limit">達成アイコンの条件</label><select class="select" id="homework-medal-limit">${[0,1,2,3,4,5].map(n=>`<option value="${n}" ${n===medalData.limit?'selected':''}>1か月の忘れ ${n}回以内</option>`).join('')}</select><span class="muted small">条件を満たす名前の横に「${esc(state.rewardIcon)}」だけを表示します。</span></div></section>${cards}`);
  ...
  async function homeworkMedalData(classId){const roster=(await rosterForClass(classId)).filter(row=>!submissionExempt(row)),allowed=new Set(roster.map(row=>row.student.id)),records=(await ClassDB.getAllByIndex('records','classId',classId)).filter(item=>item.type==='dailyHomework'&&allowed.has(item.studentId)&&item.date>=recentMonthStart()&&item.date<=today()&&!item.deletedAt&&item.status!=='absent');const counts=new Map();records.filter(item=>item.forgottenAt||item.status==='forgotten').forEach(item=>counts.set(item.studentId,(counts.get(item.studentId)||0)+1));const limit=Number(await ClassDB.getMeta('homeworkMedalLimit',0));return{limit,counts,hasRecords:records.length>0,medals:new Set(records.length?roster.filter(row=>(counts.get(row.student.id)||0)<=limit).map(row=>row.student.id):[])};}

  async function pupilAlerts(classItem){
    const records=(await ClassDB.getAllByIndex('records','classId',classItem.id)).filter(item=>!item.deletedAt),roster=(await rosterForClass(classItem.id,true)).filter(row=>!submissionExempt(row)),alerts=new Map(roster.map(row=>[row.student.id,[]]));
    if(state.pupilOverviewVisibility.daily)records.filter(item=>item.type==='dailyHomework'&&withinCurrentWeek(item.date)&&item.date<=today()&&item.status==='forgotten'&&!item.resolvedAt).forEach(item=>alerts.get(item.studentId)?.push({kind:'daily',text:pupilText(`${relativeHomeworkLabel(item.date)}の宿題が出ていません`,`${pupilHomeworkDateLabel(item.date)}の しゅくだいを だしてね`)}));
    ...
    if(state.pupilOverviewVisibility.weekly)for(const occurrence of weekly)for(const row of roster)if(!weeklySubmissions.some(item=>item.occurrenceId===occurrence.id&&item.studentId===row.student.id&&item.status==='submitted'))alerts.get(row.student.id).push({kind:'weekly',text:pupilText(`今週の${occurrence.title}が出ていません`,`${occurrence.title}を だしてね`)});
    ...
    if(state.pupilOverviewVisibility.occasional)for(const item of items)for(const row of roster)if(!submissions.some(record=>record.itemId===item.id&&record.studentId===row.student.id&&record.status==='submitted'))alerts.get(row.student.id).push({kind:'occasional',text:pupilText(`${item.title}の提出物が出ていません`,`${item.title}を だしてね`)});
  ...
  async function renderPupilDaily(){
    ...
    const roster=(await rosterForClass(classItem?.id,true,date)).filter(row=>!submissionExempt(row));const records=await dailyRecords(classItem?.id,date);const map=new Map(records.map(record=>[record.studentId,record]));const medalData=await homeworkMedalData(classItem.id);
    ...
    const totals={submitted:0,forgotten:0,unconfirmed:0,absent:0};roster.forEach(row=>{const status=map.get(row.student.id)?.status||'unconfirmed';totals[status]=(totals[status]||0)+1;});
    document.getElementById('pupil-content').innerHTML=`<div class="pupil-current-date">${esc(pupilDateText(date))}</div><div class="status-legend"><span class="legend-submitted">${esc(pupilStatusLabel('submitted'))}</span><span class="legend-forgotten">${esc(pupilStatusLabel('forgotten'))}</span><span class="legend-absent">${esc(pupilStatusLabel('absent'))}</span><span class="legend-unconfirmed">${esc(pupilStatusLabel('unconfirmed'))}</span></div><div class="summary-row"><span>${esc(pupilText('提出','だした'))} <strong>${totals.submitted}</strong></span><span>${esc(pupilText('忘れた','わすれた'))} <strong>${totals.forgotten}</strong></span><span>${esc(pupilText('欠席','おやすみ'))} <strong>${totals.absent}</strong></span><span>${esc(pupilText('未確認','まだ'))} <strong>${totals.unconfirmed}</strong></span></div>${roster.length?pupilStudentGrid(classItem,roster,map,'daily',priorForgotten,state.pupilOverviewVisibility.reward?medalData.medals:new Set()):`<section class="panel"><h2>${esc(pupilText('名簿が未登録です','なまえが とうろく されていません'))}</h2><p class="muted">${esc(pupilText('右上の歯車から教師認証し、名簿を登録してください。','せんせいに しらせてね。'))}</p></section>`}`;
  ...
  async function renderPupilWeekly(){
    const classItem=selectedClass();const roster=(await rosterForClass(classItem.id,true)).filter(row=>!submissionExempt(row));const data=await weeklyData(classItem.id);const occurrences=data.occurrences;const missingRecurring=missingRecurringWeeks(data);const medalData=await homeworkMedalData(classItem.id);
  ...
  async function renderPupilOccasional(){
    const classItem=selectedClass();const roster=(await rosterForClass(classItem.id,true)).filter(row=>!submissionExempt(row));const data=await occasionalData(classItem.id);const items=data.items.filter(item=>!item.archived);const medalData=await homeworkMedalData(classItem.id);
    ...
    document.getElementById('pupil-content').innerHTML=`<div class="pupil-current-date">${esc(pupilDateText(item.dueDate))}<span>${esc(item.title)}</span></div><div class="status-legend"><span class="legend-submitted">${esc(pupilText('✓ 提出済み','✓ だした'))}</span><span class="legend-unsubmitted">${esc(pupilStatusLabel('unsubmitted'))}</span></div><div class="summary-row"><span>${esc(pupilText('名前を押して提出を記録します。','じぶんの なまえを おしてね。'))}</span></div>${pupilStudentGrid(classItem,roster,map,'occasional',new Set(),state.pupilOverviewVisibility.reward?medalData.medals:new Set())}`;
  ...
  async function handleDailyTap(studentId,date=state.pupilDate||today(),mode='pupil'){
    ...
    else await ClassDB.put('records',{...(current||{}),id,type:'dailyHomework',classId:classItem.id,studentId,date,status:next,resolvedAt:null,forgottenAt:dailyForgottenAt(current,next)});
  ...
  async function renderSettings(){
    ...
    app.innerHTML=`<div class="app-shell">${headerHtml('設定')}<main class="page">
      <nav class="settings-nav" aria-label="設定項目">${[['guide','設定トップ'],['classes','クラス・児童'],['appearance','日常の表示・入力'],['year','年度・安全'],['data','データ管理'],['help','使い方・FAQ']].map(([id,label])=>`<button type="button" class="settings-tab" data-settings-tab="${id}" aria-selected="${id==='appearance'?['appearance','tags','prompt'].includes(state.settingsTab):state.settingsTab===id}" title="${label}を開く">${label}</button>`).join('')}</nav>
