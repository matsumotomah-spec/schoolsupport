"use strict";
  ...
  function operationTipHtml(summary,detail){return `<details class="operation-tip"><summary>${esc(summary)}</summary><p>${esc(detail)}</p></details>`;}
  async function dashboardHtml(counts){const history=await ClassDB.getMeta('syncHistory',[]),last=history[0],lastLabel=last?`${last.kindLabel||last.kind}・${new Date(last.at).toLocaleString('ja-JP',{month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit'})}`:'まだありません',items=[['毎日の宿題',counts.daily],['週宿題',counts.weekly],['提出物',counts.occasional],['児童メモ',counts.memo]].filter(item=>item[1]>0),summary=items.length?items.map(([label,count])=>`${label} ${count}人`).join('　／　'):'対応が必要な項目はありません';return`<section class="dashboard panel compact-dashboard"><div class="dashboard-heading"><div><h2>今日の確認</h2><p class="dashboard-summary">${esc(summary)}</p></div><div class="dashboard-sync"><span class="sync-dot ${last?'synced':''}"></span><span>最終同期：${esc(lastLabel)}</span></div></div></section>`;}
  ...
  function openHomeworkDisplaySettings(){
    openDialog(`<h2>宿題の表示設定</h2><p class="muted">児童用一覧に表示する情報と、条件達成時のアイコンを選びます。</p><form id="homework-display-form"><label class="check-row"><input type="checkbox" id="show-monthly-forgotten" ${state.showMonthlyForgotten?'checked':''}> 「この1か月で○回忘れ」を表示する</label><div class="field section"><span class="field-label">条件達成アイコン</span><div class="reward-icon-choices">${REWARD_ICONS.map(icon=>`<label><input type="radio" name="reward-icon" value="${icon}" ${icon===state.rewardIcon?'checked':''}><span>${icon}</span></label>`).join('')}</div><span class="muted small">文字の称号は付けず、選んだアイコンだけを名前の横に表示します。</span></div><div class="dialog-actions"><button type="button" class="button" id="homework-display-cancel">キャンセル</button><button type="button" class="button" id="open-full-appearance">機能アイコンも設定</button><button type="submit" class="button primary">保存</button></div></form>`);
    ...
    document.getElementById('homework-display-form').addEventListener('submit',async event=>{event.preventDefault();state.showMonthlyForgotten=document.getElementById('show-monthly-forgotten').checked;state.rewardIcon=document.querySelector('input[name="reward-icon"]:checked')?.value||'✨';await ClassDB.setMeta('showMonthlyForgotten',state.showMonthlyForgotten);await ClassDB.setMeta('homeworkRewardIcon',state.rewardIcon);closeDialog();showToast('宿題の表示設定を保存しました');renderHome();});
  }
  ...
  async function pupilAlerts(classItem){
    ...
    records.filter(item=>item.type==='dailyHomework'&&withinCurrentWeek(item.date)&&item.date<=today()&&item.status==='forgotten'&&!item.resolvedAt).forEach(item=>alerts.get(item.studentId)?.push({kind:'daily',text:`${relativeHomeworkLabel(item.date)}の宿題が出ていません`}));
    ...
    for(const occurrence of weekly)for(const row of roster)if(!weeklySubmissions.some(item=>item.occurrenceId===occurrence.id&&item.studentId===row.student.id&&item.status==='submitted'))alerts.get(row.student.id).push({kind:'weekly',text:`今週の${occurrence.title}が出ていません`});
    ...
    for(const item of items)for(const row of roster)if(!submissions.some(record=>record.itemId===item.id&&record.studentId===row.student.id&&record.status==='submitted'))alerts.get(row.student.id).push({kind:'occasional',text:`${item.title}の提出物が出ていません`});
  ...
  async function renderPupilAll(){
    ...
    document.getElementById('pupil-content').innerHTML=`<section class="pupil-overview-head"><h1>提出するもの一覧</h1><p>名前を探して、出ていないものを確認してください。提出の操作は上の各ページで行います。</p></section><section class="pupil-alert-grid">${roster.map(row=>{const items=alerts.get(row.student.id),count=medalData.counts.get(row.student.id)||0;return`<article class="pupil-alert-card ${items.length?'needs-action':'all-clear'}"><div class="pupil-alert-name"><strong>${esc(row.student.name)}</strong>${medalData.medals.has(row.student.id)?rewardIconHtml():''}</div>${state.showMonthlyForgotten?`<div class="monthly-forgotten">この1か月で ${count}回忘れ</div>`:''}${items.length?`<div class="pupil-alert-list">${items.map(item=>`<button type="button" data-alert-tool="${item.kind}">! ${esc(item.text)}</button>`).join('')}</div>`:'<div class="all-clear-label">✓ 今のところ未提出はありません</div>'}</article>`;}).join('')}</section>${!roster.length?'<section class="panel"><h2>名簿が未登録です</h2><p>先生に知らせてください。</p></section>':''}<p class="pupil-overview-summary">未提出・忘れの表示がある児童：${needing.length}人</p>`;
  ...
  async function rosterForClass(classId,useSeatOrder=false,atDate=null){
    ...
    const enrollments=(await ClassDB.getAllByIndex('enrollments','classId',classId)).filter(item=>atDate?(item.startDate||state.year.startDate)<=atDate&&(!item.endDate||item.endDate>=atDate):!item.endDate).sort((a,b)=>{const ai=order.indexOf(a.studentId),bi=order.indexOf(b.studentId);if(ai>=0||bi>=0)return(ai<0?999:ai)-(bi<0?999:bi);return(a.number||999)-(b.number||999);});
  ...
  async function enrollmentPeriodsForStudent(classId,studentId){return(await ClassDB.getAllByIndex('enrollments','studentId',studentId)).filter(item=>item.classId===classId).sort((a,b)=>String(b.startDate||'').localeCompare(String(a.startDate||'')));}
  ...
  async function renderSettings(){
    ...
    app.innerHTML=`<div class="app-shell">${headerHtml('設定')}<main class="page">
      <nav class="settings-nav" aria-label="設定項目">${[['guide','設定トップ'],['classes','クラス・名簿'],['year','年度・ログイン'],['appearance','画面の見た目'],['tags','メモ・賞状の選択肢'],['prompt','所見素材'],['data','保存・端末間共有'],['help','使い方・FAQ']].map(([id,label])=>`<button type="button" class="settings-tab" data-settings-tab="${id}" aria-selected="${state.settingsTab===id}" title="${label}を開く">${label}</button>`).join('')}</nav>
