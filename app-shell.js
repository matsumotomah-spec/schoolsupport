"use strict";
  ...
  async function renderHome(){
    ...
    document.querySelectorAll('[data-class-id]').forEach(button=>button.addEventListener('click',()=>navigateSafely(async()=>{await selectClassForWork(button.dataset.classId);renderHome();})));
  ...
  function teacherToolShell(title,body,actions=''){
    ...
    return friendlyTerms(`<div class="app-shell">${headerHtml(title,actions)}<main class="page" data-page-key="${esc(key)}">${onboardingBannerHtml()}${purpose}<div class="page-content" data-page-content="${esc(key)}">${body}</div></main>${key==='student'?'':teacherFooter(key)}</div>`);
  }
  ...
  async function teacherRosterCards(classId,records=[],options={}){
    ...
    allRecords.filter(item=>item.type==='memo'&&normalRecord(item)).forEach(item=>{if(!lastMemo.has(item.studentId)||lastMemo.get(item.studentId)<item.date)lastMemo.set(item.studentId,item.date);});
    const card=row=>{
  ...
  async function memoShortages(classItem,baseDate=today()){
    ...
    const records=(await ClassDB.getAllByIndex('records','classId',classItem.id)).filter(item=>item.type==='memo'&&item.date>=termStart&&item.date<=termEnd&&normalRecord(item));
  ...
  async function renderTeacherDaily(){
    ...
    const roster=await rosterForClass(classItem.id);const records=await dailyRecords(classItem.id,date);const classRecords=(await ClassDB.getAllByIndex('records','classId',classItem.id)).filter(normalRecord);const priorForgotten=new Set(classRecords.filter(item=>item.type==='dailyHomework'&&item.date>=currentWeekStart(date)&&item.date<date&&['forgotten','partialForgotten'].includes(item.status)&&!item.resolvedAt).map(item=>item.studentId));const medalData=await homeworkMedalData(classItem.id);
    ...
    app.innerHTML=teacherToolShell('毎日の宿題',`${pupilDateNav(date,'teacher-daily-prev','teacher-daily-next')}<section class="panel daily-guide"><div class="toolbar-line"><h2>提出状況</h2>${teacherOrderControlHtml()}</div><div class="summary-row"><span>提出 <strong>${totals.submitted}</strong></span><span>忘れた <strong>${totals.forgotten}</strong></span><span>一部忘れた <strong>${totals.partialForgotten}</strong></span><span>欠席 <strong>${totals.absent}</strong></span><span>未確認 <strong>${totals.unconfirmed}</strong></span></div></section>${cards}<section class="panel homework-after-roster"><h2>操作と設定</h2>${operationTipHtml('児童カードの押し方を見る','児童名は、提出 → 忘れた → 欠席 → 未確認の順で切り替わります。「一部忘れた」は児童用画面または詳細から記録できます。')}<details class="daily-utility"><summary>欠席設定・達成アイコンの条件</summary><div class="button-row section"><button type="button" class="button" id="daily-absence-setting">欠席をまとめて設定</button></div><div class="medal-setting"><label for="homework-medal-limit">達成アイコンの条件</label><select class="select" id="homework-medal-limit">${[0,1,2,3,4,5].map(n=>`<option value="${n}" ${n===medalData.limit?'selected':''}>1か月の忘れ ${n}回以内</option>`).join('')}</select><span class="muted small">一部忘れた（0.5回）が1件でもある児童は、今月の達成アイコン対象外です。</span></div></details><button type="button" class="button homework-help-button" data-page-purpose-help="daily">この画面の使い方</button></section>`);
  ...
  async function pupilAlerts(classItem){
    const records=(await ClassDB.getAllByIndex('records','classId',classItem.id)).filter(normalRecord),roster=(await rosterForClass(classItem.id,true)).filter(row=>!submissionExempt(row)),visibleRoster=roster.filter(row=>!pupilStatusHidden(row)),alerts=new Map(roster.map(row=>[row.student.id,[]])),visibleDailyIds=new Set(visibleRoster.map(row=>row.student.id));
  ...
  async function renderPupilDaily(){
    ...
    const classRecords=(await ClassDB.getAllByIndex('records','classId',classItem.id)).filter(normalRecord);const priorForgotten=new Set(classRecords.filter(item=>item.type==='dailyHomework'&&item.date>=currentWeekStart(date)&&item.date<date&&['forgotten','partialForgotten'].includes(item.status)&&!item.resolvedAt).map(item=>item.studentId));
  ...
  async function dailyRecords(classId,date){const records=await ClassDB.getAllByIndex('records','classId',classId);return records.filter(item=>item.type==='dailyHomework'&&item.date===date&&normalRecord(item));}
  const dailyTapQueues=new Map();
  async function handleDailyTap(studentId,date=state.pupilDate||today(),mode='pupil'){
    const classId=selectedClass()?.id||'',key=`${mode}:${classId}:${date}:${studentId}`;
    const previous=dailyTapQueues.get(key)||Promise.resolve();
    const current=previous.catch(()=>{}).then(()=>handleDailyTapUnlocked(studentId,date,mode));
    dailyTapQueues.set(key,current);
    try{return await current;}finally{if(dailyTapQueues.get(key)===current)dailyTapQueues.delete(key);}
  }
  async function handleDailyTapUnlocked(studentId,date=state.pupilDate||today(),mode='pupil'){
    if(mode==='pupil'&&rolloverDue()&&!state.rolloverContinue){requireTeacher(()=>confirmOldYearContinuation(()=>handleDailyTapUnlocked(studentId,date,mode)));return;}
    const classItem=selectedClass();
    const id=`daily_${classItem.id}_${date}_${studentId}`;
    const current=await ClassDB.get('records',id);
    const all=(await ClassDB.getAllByIndex('records','studentId',studentId)).filter(normalRecord);
    const prior=all.filter(item=>item.type==='dailyHomework'&&item.classId===classItem.id&&item.date>=currentWeekStart(date)&&item.date<date&&['forgotten','partialForgotten'].includes(item.status)&&!item.resolvedAt).sort((a,b)=>a.date.localeCompare(b.date));
    ...
    markFeedback(studentId,next);const message=next==='submitted'?'提出にしました':next==='forgotten'?'忘れたにしました':next==='partialForgotten'?'一部忘れたにしました':next==='absent'?'欠席にしました':'未確認に戻しました';await renderDailyContext(mode);
      if(mode==='teacher')showUndoToast(message,async()=>{if(current){await ClassDB.put('records',current);await ClassDB.remove('trash',`trash_${id}`);}else await moveToTrash(await ClassDB.get('records',id));await renderDailyContext(mode);});else showToast(message);
  }
  ...
  async function openDailyAbsenceDialog(date){
    const classItem=selectedClass(),roster=await rosterForClass(classItem.id,true,date),records=await dailyRecords(classItem.id,date),absentIds=new Set(records.filter(item=>item.status==='absent').map(item=>item.studentId)),byId=new Map(roster.map(row=>[row.student.id,row])),layout=Array.isArray(classItem.activeSeatLayout)&&classItem.activeSeatLayout.length?classItem.activeSeatLayout:roster.map(row=>row.student.id),seatItem=row=>submissionExempt(row)?`<span class="daily-absence-seat exchange-seat-only"><span><small>${esc(row.enrollment?.number||'')}</small>${esc(row.student.name)}<em>座席のみ</em></span></span>`:`<label class="daily-absence-seat"><input type="checkbox" data-daily-absent="${row.student.id}" ${absentIds.has(row.student.id)?'checked':''}><span><small>${esc(row.enrollment?.number||'')}</small>${esc(row.student.name)}</span></label>`,seatGrid=layout.filter(id=>id&&byId.has(id)).map(id=>seatItem(byId.get(id))).join('');
    openDialog(`<h2>${esc(shortJpDate(date))}の欠席を設定</h2><p class="muted">座席順で選びます。画面の幅に合わせて並び替わるため、横にスクロールせずに選べます。欠席にした児童は、この日の宿題忘れ・月間の忘れ回数・児童用の忘れ物表示に含めません。</p><form id="daily-absence-form"><div class="daily-absence-grid">${seatGrid}</div><div class="dialog-actions"><button type="button" class="button" id="daily-absence-cancel">キャンセル</button><button type="submit" class="button primary" id="daily-absence-save">保存する</button></div></form>`);
    document.getElementById('daily-absence-cancel').addEventListener('click',requestDialogClose);document.getElementById('daily-absence-form').addEventListener('submit',event=>{event.preventDefault();const button=document.getElementById('daily-absence-save');runOnce(button,async()=>{const selected=new Set([...document.querySelectorAll('[data-daily-absent]:checked')].map(input=>input.dataset.dailyAbsent));for(const row of roster){const current=records.find(item=>item.studentId===row.student.id);if(selected.has(row.student.id)&&current?.status!=='absent')await setDailyStatus(row.student.id,date,'absent');if(!selected.has(row.student.id)&&current?.status==='absent')await moveToTrash(current);}closeDialog();showToast(selected.size?`${selected.size}人を欠席にしました`:'欠席設定を解除しました');renderTeacherDaily();});});
  }
