"use strict";
  ...
  async function pupilAlerts(classItem){
    ...
    if(state.pupilOverviewVisibility.daily)records.filter(item=>item.type==='dailyHomework'&&visibleDailyIds.has(item.studentId)&&withinCurrentWeek(item.date)&&item.date<=today()&&['forgotten','partialForgotten'].includes(item.status)&&!item.resolvedAt).forEach(item=>alerts.get(item.studentId)?.push({kind:'daily',text:item.status==='partialForgotten'?pupilText(`${relativeHomeworkLabel(item.date)}の宿題に一部忘れがあります`,`${pupilHomeworkDateLabel(item.date)}の しゅくだいを たしかめてね`):pupilText(`${relativeHomeworkLabel(item.date)}の宿題が出ていません`,`${pupilHomeworkDateLabel(item.date)}の しゅくだいを だしてね`)}));
