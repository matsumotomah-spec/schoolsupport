"use strict";
  ...
  async function renderPupilAll(){
    ...
    document.getElementById('pupil-content').innerHTML=`<section class="pupil-overview-head"><h1>${esc(pupilText('提出するもの一覧','だすもの'))}</h1><p>${esc(pupilText('名前を探して、出ていないものを確認してください。提出の操作は上の各ページで行います。','じぶんの なまえを さがしてね。だすものを たしかめよう。'))}</p></section><section class="pupil-alert-grid">${roster.map(row=>{const items=alerts.get(row.student.id),count=medalData.counts.get(row.student.id)||0;return`<article class="pupil-alert-card ${items.length?'needs-action':'all-clear'}"><div class="pupil-alert-name"><strong>${esc(row.student.name)}</strong>${state.pupilOverviewVisibility.reward&&medalData.medals.has(row.student.id)?rewardIconHtml():''}</div>${state.pupilOverviewVisibility.monthly?`<div class="monthly-forgotten">${esc(pupilText(`この1か月で ${count}回忘れ`,`この 1かげつで ${count}かい わすれた`))}</div>`:''}${items.length?`<div class="pupil-alert-list">${items.map(item=>`<button type="button" data-alert-tool="${item.kind}">! ${esc(item.text)}</button>`).join('')}</div>`:`<div class="all-clear-label">✓ ${esc(pupilText('表示中の項目に未提出はありません','だしていないものは ありません'))}</div>`}</article>`;}).join('')}</section>${!roster.length?`<section class="panel"><h2>${esc(pupilText('名簿が未登録です','なまえが とうろく されていません'))}</h2><p>${esc(pupilText('先生に知らせてください。','せんせいに しらせてね。'))}</p></section>`:''}<p class="pupil-overview-summary">${esc(pupilText(`表示中の項目で確認が必要な児童：${needing.length}人`,`たしかめる ひと：${needing.length}にん`))}</p>`;
  ...
  async function renderPupilDaily(){
    ...
    document.getElementById('pupil-content').innerHTML=`<div class="pupil-current-date">${esc(pupilDateText(date))}</div><div class="status-legend"><span class="legend-submitted">${esc(pupilStatusLabel('submitted'))}</span><span class="legend-forgotten">${esc(pupilStatusLabel('forgotten'))}</span><span class="legend-unconfirmed">${esc(pupilStatusLabel('unconfirmed'))}</span></div><div class="summary-row"><span>${esc(pupilText('提出','だした'))} <strong>${totals.submitted}</strong></span><span>${esc(pupilText('忘れた','わすれた'))} <strong>${totals.forgotten}</strong></span><span>${esc(pupilText('未確認','まだ'))} <strong>${totals.unconfirmed}</strong></span></div>${roster.length?pupilStudentGrid(classItem,roster,map,'daily',priorForgotten,medalData.medals):`<section class="panel"><h2>${esc(pupilText('名簿が未登録です','なまえが とうろく されていません'))}</h2><p class="muted">${esc(pupilText('右上の歯車から教師認証し、名簿を登録してください。','せんせいに しらせてね。'))}</p></section>`}`;
