"use strict";
  ...
  document.addEventListener('click',event=>{if(!event.target.closest('[data-home-menu]'))return;setTimeout(()=>{const drawer=document.getElementById('home-menu-drawer');if(!drawer)return;const classButton=drawer.querySelector('[data-home-menu-settings="classes"]'),appearanceButton=drawer.querySelector('[data-home-menu-settings="appearance"]'),dataButton=drawer.querySelector('[data-home-menu-settings="data"]');if(classButton){classButton.querySelector('strong').textContent='クラス・名簿';}if(appearanceButton){appearanceButton.querySelector('strong').textContent='表示・入力';}if(dataButton){dataButton.dataset.homeMenuSettings='safety';dataButton.querySelector('strong').textContent='データ・安全';dataButton.querySelector('small').textContent='同期、保存、年度、PIN';}},0);});
  ...
  function teacherFooter(active){const classItem=selectedClass(),allowed=!classItem?.isOwn&&!isSupportClass(classItem)?['assessment','records','tests','occasional','grades']:['daily','weekly','assessment','records','tests','occasional','certificate','grades'];const labels={daily:'毎日の宿題',weekly:'週宿題',certificate:'ミニ賞状',records:'児童の記録',assessment:'ノート評価',tests:'小テスト',grades:'成績管理',occasional:'提出物'};return`<nav class="teacher-footer" aria-label="日常機能" style="--footer-count:${allowed.length}">${allowed.map(id=>`<button type="button" data-footer-tool="${id}" aria-current="${active===id?'page':'false'}" title="${labels[id]}へ切り替える"><span>${featureIcon(id)}</span>${labels[id]}</button>`).join('')}</nav>`;}
