"use strict";
  ...
  async function renderPupil(tool=state.pupilTool||'daily'){
    ...
    const nav=`<nav class="pupil-nav" aria-label="${esc(pupilText('提出画面','ていしゅつ がめん'))}"><button type="button" data-pupil-tool="all" aria-selected="${tool==='all'}">${esc(pupilText('一覧','みる'))}</button><button type="button" data-pupil-tool="daily" aria-selected="${tool==='daily'}">${esc(pupilText('毎日の宿題','きょうの しゅくだい'))}</button><button type="button" data-pupil-tool="weekly" aria-selected="${tool==='weekly'}">${esc(pupilText('週宿題','しゅうの しゅくだい'))}</button><button type="button" data-pupil-tool="occasional" aria-selected="${tool==='occasional'}">${esc(pupilText('提出物','ていしゅつもの'))}</button></nav>`;
  ...
  async function homeworkMedalData(classId){const records=(await ClassDB.getAllByIndex('records','classId',classId)).filter(item=>item.type==='dailyHomework'&&item.date>=recentMonthStart()&&item.date<=today()&&!item.deletedAt&&item.status!=='absent');const counts=new Map();records.filter(item=>item.forgottenAt||item.status==='forgotten').forEach(item=>counts.set(item.studentId,(counts.get(item.studentId)||0)+1));const limit=Number(await ClassDB.getMeta('homeworkMedalLimit',0));const roster=await rosterForClass(classId);return{limit,counts,hasRecords:records.length>0,medals:new Set(records.length?roster.filter(row=>(counts.get(row.student.id)||0)<=limit).map(row=>row.student.id):[])};}
  ...
  async function renderPupilDaily(){
    ...
    document.getElementById('pupil-content').innerHTML=`<div class="pupil-current-date">${esc(pupilDateText(date))}</div><div class="status-legend"><span class="legend-submitted">${esc(pupilStatusLabel('submitted'))}</span><span class="legend-forgotten">${esc(pupilStatusLabel('forgotten'))}</span><span class="legend-absent">${esc(pupilStatusLabel('absent'))}</span><span class="legend-unconfirmed">${esc(pupilStatusLabel('unconfirmed'))}</span></div><div class="summary-row"><span>${esc(pupilText('提出','だした'))} <strong>${totals.submitted}</strong></span><span>${esc(pupilText('忘れた','わすれた'))} <strong>${totals.forgotten}</strong></span><span>${esc(pupilText('欠席','おやすみ'))} <strong>${totals.absent}</strong></span><span>${esc(pupilText('未確認','まだ'))} <strong>${totals.unconfirmed}</strong></span></div>${roster.length?pupilStudentGrid(classItem,roster,map,'daily',priorForgotten,medalData.medals):`<section class="panel"><h2>${esc(pupilText('名簿が未登録です','なまえが とうろく されていません'))}</h2><p class="muted">${esc(pupilText('右上の歯車から教師認証し、名簿を登録してください。','せんせいに しらせてね。'))}</p></section>`}`;
  ...
  async function renderPupilOccasional(){
    ...
    document.getElementById('pupil-content').innerHTML=`<div class="pupil-current-date">${esc(pupilDateText(item.dueDate))}<span>${esc(item.title)}</span></div><div class="status-legend"><span class="legend-submitted">${esc(pupilText('✓ 提出済み','✓ だした'))}</span><span class="legend-unsubmitted">${esc(pupilStatusLabel('unsubmitted'))}</span></div><div class="summary-row"><span>${esc(pupilText('名前を押して提出を記録します。','じぶんの なまえを おしてね。'))}</span></div>${pupilStudentGrid(classItem,roster,map,'occasional',new Set(),medalData.medals)}`;
  ...
  async function openDailyAbsenceDialog(date){
    const classItem=selectedClass(),roster=await rosterForClass(classItem.id),records=await dailyRecords(classItem.id,date),absentIds=new Set(records.filter(item=>item.status==='absent').map(item=>item.studentId));
    openDialog(`<h2>${esc(shortJpDate(date))}の欠席を設定</h2><p class="muted">欠席にした児童は、この日の宿題忘れ・月間の忘れ回数・児童用の忘れ物表示に含めません。</p><form id="daily-absence-form"><div class="check-list">${roster.map(row=>`<label class="check-row"><input type="checkbox" data-daily-absent="${row.student.id}" ${absentIds.has(row.student.id)?'checked':''}> ${esc(row.student.name)}</label>`).join('')}</div><div class="dialog-actions"><button type="button" class="button" id="daily-absence-cancel">キャンセル</button><button type="submit" class="button primary" id="daily-absence-save">保存する</button></div></form>`);
