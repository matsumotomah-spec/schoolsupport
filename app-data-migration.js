"use strict";
  ...
  function classTrashEntryFor(classItem,{enrollments,records,students}){const deletedAt=ClassDB.now(),deviceId=ClassDB.deviceId(),{seatingSettings,...classData}=classItem;return{id:`trash_class_${classItem.id}_${deletedAt}`,kind:'classBundle',classBundle:{class:classData,seatingSettings:seatingSettings||null,enrollments:[...enrollments],records:[...records],students:[...students]},deletedAt,purgeAfter:new Date(Date.now()+30*86400000).toISOString(),createdAt:deletedAt,updatedAt:deletedAt,deviceId};}
  function trashBelongsToCurrentYear(item){if(item?.kind==='classBundle')return item.classBundle?.class?.yearId===state.year?.id;const classId=item?.record?.classId;return Boolean(classId&&state.classes.some(classItem=>classItem.id===classId));}
  async function restoreTrashRecord(trashId){
    const item=await ClassDB.get('trash',trashId);
    if(!item||!trashBelongsToCurrentYear(item))return 0;
    const timestamp=ClassDB.now(),deviceId=ClassDB.deviceId();
    if(item.kind==='classBundle'&&item.classBundle){
      const bundle=item.classBundle,restoredClass={...bundle.class,updatedAt:timestamp,deviceId};
      if(await ClassDB.get('classes',restoredClass.id))return 0;
      const [allEnrollments,allStudents,allRecords]=await Promise.all([ClassDB.getAll('enrollments'),ClassDB.getAll('students'),ClassDB.getAll('records')]);
      const sharedStudentIds=new Set(allEnrollments.filter(row=>row.classId!==restoredClass.id).map(row=>row.studentId));
      const existingStudentIds=new Set(allStudents.map(row=>row.id));
      const existingEnrollmentIds=new Set(allEnrollments.map(row=>row.id));
      const existingRecordIds=new Set(allRecords.map(row=>row.id));
      const students=(bundle.students||[]).filter(row=>!sharedStudentIds.has(row.id)||!existingStudentIds.has(row.id)).map(row=>({...row,updatedAt:timestamp,deviceId}));
      const enrollments=(bundle.enrollments||[]).filter(row=>!existingEnrollmentIds.has(row.id)).map(row=>({...row,updatedAt:timestamp,deviceId}));
      const records=(bundle.records||[]).filter(row=>!existingRecordIds.has(row.id)).map(row=>{const restored={...row,updatedAt:timestamp,deviceId};delete restored.deletedAt;return restored;});
      const puts={classes:[restoredClass],enrollments,students,records,meta:[{key:'selectedClassId',value:restoredClass.id,updatedAt:timestamp,deviceId},...(bundle.seatingSettings?[bundle.seatingSettings]:[])]};
      await ClassDB.applyBatch({puts,deletes:{trash:[item.id]}});
      state.classes=[...state.classes,restoredClass];
      state.selectedClassId=restoredClass.id;
      return 1;
    }
    if(!item.record)return 0;let items=[item],relatedRecords=[];const record=item.record,puts={};
  ...
  async function confirmDeleteAllData(){
    ...
    document.getElementById('delete-all-cancel').addEventListener('click',requestDialogClose);
  ...
  async function confirmDeleteClassRecords(){const classItem=selectedClass();if(!classItem)return;const records=await ClassDB.getAllByIndex('records','classId',classItem.id);openDialog(`<h2>${esc(classItem.name)}の記録を削除しますか</h2><p>宿題・提出物・評価・メモなど ${records.length}件を30日間ごみ箱へ移します。名簿とクラス設定は残ります。</p><div class="dialog-actions"><button type="button" class="button" id="records-delete-cancel">キャンセル</button><button type="button" class="button danger" id="records-delete-confirm">${records.length}件を削除</button></div>`);document.getElementById('records-delete-cancel').addEventListener('click',requestDialogClose);document.getElementById('records-delete-confirm').addEventListener('click',async()=>{const timestamp=ClassDB.now(),deviceId=ClassDB.deviceId(),meta={key:'lastLegacyMigration',value:null,updatedAt:timestamp,deviceId};await ClassDB.applyBatch({puts:{trash:records.map(trashEntryFor),meta:[meta]},deletes:{records:records.map(record=>record.id)}});closeDialog();showToast(`${records.length}件をごみ箱へ移動しました`);renderDataExchange(true);});}

  async function confirmDeleteCurrentClass(){const classItem=selectedClass();if(!classItem)return;const roster=(await rosterForClass(classItem.id)).length,records=(await ClassDB.getAllByIndex('records','classId',classItem.id)).length;openDialog(`<h2>${esc(classItem.name)}を削除しますか</h2><p>名簿 ${roster}人、記録 ${records}件を30日間ごみ箱へ移します。他年度や他クラスのデータは変更しません。</p><p class="notice">ごみ箱からクラス・名簿・記録をまとめて復元できます。</p><div class="dialog-actions"><button type="button" class="button" id="class-delete-cancel">キャンセル</button><button type="button" class="button danger" id="class-delete-confirm">クラスをごみ箱へ移す</button></div>`);document.getElementById('class-delete-cancel').addEventListener('click',requestDialogClose);document.getElementById('class-delete-confirm').addEventListener('click',async()=>{const enrollments=await ClassDB.getAllByIndex('enrollments','classId',classItem.id),classRecords=await ClassDB.getAllByIndex('records','classId',classItem.id),allEnrollments=await ClassDB.getAll('enrollments'),studentIds=new Set(enrollments.map(item=>item.studentId)),usedElsewhere=new Set(allEnrollments.filter(item=>item.classId!==classItem.id).map(item=>item.studentId)),students=await Promise.all([...studentIds].filter(id=>!usedElsewhere.has(id)).map(id=>ClassDB.get('students',id))),seatingSettings=await ClassDB.get('meta',`seatingSettings_${classItem.id}`),bundle=classTrashEntryFor({...classItem,seatingSettings},{enrollments,records:classRecords,students:students.filter(Boolean)});state.classes=state.classes.filter(item=>item.id!==classItem.id);state.selectedClassId=state.classes[0]?.id||null;const timestamp=ClassDB.now(),deviceId=ClassDB.deviceId(),meta=[{key:'selectedClassId',value:state.selectedClassId,updatedAt:timestamp,deviceId},{key:'lastLegacyMigration',value:null,updatedAt:timestamp,deviceId}];await ClassDB.applyBatch({puts:{trash:[bundle],meta},deletes:{records:classRecords.map(item=>item.id),enrollments:enrollments.map(item=>item.id),classes:[classItem.id],students:[...studentIds].filter(id=>!usedElsewhere.has(id)),meta:[`seatingSettings_${classItem.id}`]}});closeDialog();if(!state.classes.length){state.settingsTab='classes';state.classSettingsView='list';renderSettings();showToast('クラスをごみ箱へ移しました。必要ならデータ管理から復元できます');}else{renderHome();showToast('クラスをごみ箱へ移しました');}});}
  ...
  async function inspectSmartImportFiles(files){
    if(!files.length)return;try{
      ...
      if(!legacy.length&&tables.length===1){const file=tables[0];if(/\.xlsx$/i.test(file.name)){const sheets=testWorkbookSheets(await XlsxRosterReader.readWorkbook(file));if(sheets.some(sheet=>sheet.tests.length)){openTestImportPreview(file.name,sheets);return;}}openExternalImport(file);return;}
      openDialog(`<h2>ファイルの種類を確認しました</h2><p class="muted">紙テスト、宿題、提出物、名簿など、取り込みたい内容を選んでください。</p><div class="import-choice-list section">${legacy.length?`<button type="button" class="import-choice" id="smart-open-legacy"><strong>以前のツールのデータ</strong><span>${legacy.length}件のデータ群を検出しました</span><b>内容を確認 ›</b></button>`:''}${tables.map((file,index)=>`<button type="button" class="import-choice" data-smart-table="${index}"><strong>${esc(file.name)}</strong><span>Excelなら紙テスト、宿題、提出物を判別します</span><b>判別する ›</b></button>`).join('')}</div><div class="dialog-actions"><button type="button" class="button" id="smart-import-close">閉じる</button></div>`,'dialog-xwide');document.getElementById('smart-import-close').addEventListener('click',requestDialogClose);document.getElementById('smart-open-legacy')?.addEventListener('click',()=>{closeDialog();openLegacyMigrationReview(legacy);});document.querySelectorAll('[data-smart-table]').forEach(button=>button.addEventListener('click',async()=>{const file=tables[Number(button.dataset.smartTable)];closeDialog();if(/\.xlsx$/i.test(file.name)){const sheets=testWorkbookSheets(await XlsxRosterReader.readWorkbook(file));if(sheets.some(sheet=>sheet.tests.length)){openTestImportPreview(file.name,sheets);return;}}openExternalImport(file);}));
  ...
  function archiveRecordSummary(record){
    const labels={memo:'児童メモ',notebookAssessment:'ノート評価',certificate:'ミニ賞状',dailyHomework:'毎日の宿題',weeklySubmission:'週宿題',occasionalSubmission:'提出物',testScore:'テスト',supportRecord:'学習記録',behaviorMark:'行動の記録'};
    if(record.type==='testScore')return`${record.subject||''} ${record.title||'テスト'} ${Number.isFinite(record.total)?`${record.total}/${record.maxTotal||'—'}点`:''}`.trim();
    if(record.type==='notebookAssessment')return`${record.subject||''} ${record.unit||''} ${record.title||''} ${record.grade||record.status||''}`.trim();
    if(record.type==='certificate')return(record.tags||[]).join('・')||'渡した';
    if(record.type==='memo'||record.type==='supportRecord')return`${record.subject||''} ${record.text||''}`.trim();
    return record.title||record.status||labels[record.type]||record.type;
  }

  function openArchiveRecordBrowser(payload,classId='',studentId=''){
    const data=payload.data,classes=data.classes.filter(item=>item.yearId===payload.yearId),activeClass=classes.find(item=>item.id===classId)||classes[0];
    if(!activeClass){openDialog('<h2>確認できるクラスがありません</h2><p>保管ファイルの内容を確認してください。</p><div class="dialog-actions"><button type="button" class="button primary" id="archive-browser-close">閉じる</button></div>');document.getElementById('archive-browser-close').addEventListener('click',requestDialogClose);return;}
    const enrollments=data.enrollments.filter(item=>item.classId===activeClass.id),studentsById=new Map(data.students.map(item=>[item.id,item])),roster=enrollments.map(item=>({enrollment:item,student:studentsById.get(item.studentId)})).filter(item=>item.student).sort((a,b)=>Number(a.enrollment.number||9999)-Number(b.enrollment.number||9999)||a.student.name.localeCompare(b.student.name,'ja'));
    const activeStudent=roster.find(item=>item.student.id===studentId)?.student||roster[0]?.student||null;
    const records=activeStudent?data.records.filter(item=>item.classId===activeClass.id&&item.studentId===activeStudent.id&&!item.deletedAt).sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))):[];
    const counts={classes:classes.length,students:roster.length,records:data.records.filter(item=>item.classId===activeClass.id&&!item.deletedAt).length};
    openDialog(`<h2>${esc(payload.yearLabel||'保管年度')}を閲覧</h2><p class="notice small"><strong>閲覧専用です。</strong> 現在の年度のデータは変更しません。</p><div class="overview-stats"><div class="overview-stat"><span class="row-meta">クラス</span><strong>${counts.classes}</strong></div><div class="overview-stat"><span class="row-meta">このクラスの児童</span><strong>${counts.students}</strong></div><div class="overview-stat"><span class="row-meta">このクラスの記録</span><strong>${counts.records}</strong></div></div><div class="field section"><label for="archive-class-select">クラス</label><select class="select" id="archive-class-select">${classes.map(item=>`<option value="${esc(item.id)}" ${item.id===activeClass.id?'selected':''}>${esc(item.name)}</option>`).join('')}</select></div><div class="archive-browser-grid section"><section><h3>児童を選ぶ</h3><div class="list archive-student-list">${roster.map(item=>`<button type="button" class="list-row ${item.student.id===activeStudent?.id?'selected':''}" data-archive-student="${esc(item.student.id)}"><strong>${esc(item.enrollment.number||'—')}　${esc(item.student.name)}</strong></button>`).join('')||'<p class="muted">児童がいません。</p>'}</div></section><section><h3>${activeStudent?`${esc(activeStudent.name)}の記録`:'記録'}</h3><div class="record-list">${records.map(item=>`<article class="record-item"><strong>${esc(jpDate(item.date||''))}　${esc({memo:'児童メモ',notebookAssessment:'ノート評価',certificate:'ミニ賞状',dailyHomework:'毎日の宿題',weeklySubmission:'週宿題',occasionalSubmission:'提出物',testScore:'テスト',supportRecord:'学習記録',behaviorMark:'行動の記録'}[item.type]||item.type)}</strong><div class="row-meta">${esc(archiveRecordSummary(item))}</div></article>`).join('')||'<p class="muted">この児童の記録はありません。</p>'}</div></section></div><div class="dialog-actions"><button type="button" class="button primary" id="archive-browser-close">閉じる</button></div>`,'dialog-xwide');
    document.getElementById('archive-browser-close').addEventListener('click',requestDialogClose);
    document.getElementById('archive-class-select').addEventListener('change',event=>openArchiveRecordBrowser(payload,event.target.value,''));
    document.querySelectorAll('[data-archive-student]').forEach(button=>button.addEventListener('click',()=>openArchiveRecordBrowser(payload,activeClass.id,button.dataset.archiveStudent)));
  }

  async function inspectArchiveFileEnhanced(file){
    if(!file)return;
    try{
      const envelope=validateEncryptedEnvelope(JSON.parse(await readImportText(file,'保管ファイル')));
      openDialog(`<h2>${esc(envelope.yearLabel||'保管年度')}を開く</h2><p class="muted">保管ファイルの中だけを閲覧します。現在の年度へは取り込みません。</p><form id="archive-browser-form"><div class="field"><label for="archive-browser-method">開く方法</label><select class="select" id="archive-browser-method"><option value="password">データ保護パスワード</option><option value="recovery">緊急復旧コード</option></select></div><div class="field section"><label for="archive-browser-credential">パスワードまたは復旧コード</label><input class="input" id="archive-browser-credential" type="password" required autofocus></div><p class="error" id="archive-browser-error"></p><div class="dialog-actions"><button type="button" class="button" id="archive-browser-cancel">キャンセル</button><button type="submit" class="button primary">閲覧する</button></div></form>`);
      document.getElementById('archive-browser-cancel').addEventListener('click',requestDialogClose);
      document.getElementById('archive-browser-form').addEventListener('submit',async event=>{event.preventDefault();const error=document.getElementById('archive-browser-error'),method=document.getElementById('archive-browser-method').value,credential=document.getElementById('archive-browser-credential').value.trim();error.textContent='読み取っています…';try{const payload=validateSyncPayload(await decryptEnvelope(envelope,method==='recovery'?credential.toUpperCase():credential,method));openArchiveRecordBrowser(payload);}catch(problem){error.textContent=problem.message||'保管ファイルを読み取れませんでした';}});
    }catch(problem){openDialog(`<h2>読み込めませんでした</h2><p>${esc(problem.message||'ファイルを確認してください')}</p><div class="dialog-actions"><button type="button" class="button primary" id="archive-error-close">OK</button></div>`);document.getElementById('archive-error-close').addEventListener('click',requestDialogClose);}
  }
