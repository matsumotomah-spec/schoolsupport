"use strict";
  ...
  function behaviorViewTabs(view){return`<nav class="behavior-view-tabs" aria-label="行動の記録の表示"><button type="button" data-behavior-view="input" aria-current="${view==='input'?'page':'false'}">記録入力</button><button type="button" data-behavior-view="summary" aria-current="${view==='summary'?'page':'false'}">期間集計</button></nav>`;}
  ...
  async function renderBehavior(){
    const draft=behaviorDraft();state.route='teacher-behavior';state.activeTool='behavior';
  ...
  async function renderBehaviorInput(){
    ...
    if(!category){
      app.innerHTML=teacherToolShell('行動の記録',`${behaviorViewTabs('input')}<section class="panel behavior-category-select"><h1>記録するカテゴリーを選択</h1><p class="muted">よい姿を見つけたカテゴリーを先に選びます。○がない日は、できなかったという意味ではありません。</p>${behaviorBoundaryHint()}${behaviorCategoryButtons(counts)}</section>`);
  ...
  function wireBehaviorCommon(){wireToolHome();document.querySelectorAll('[data-behavior-view]').forEach(button=>button.addEventListener('click',()=>{behaviorDraft().view=button.dataset.behaviorView;renderBehavior();}));}
