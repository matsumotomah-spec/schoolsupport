"use strict";
  ...
  async function renderBehaviorInput(){
    ...
    app.innerHTML=teacherToolShell('児童の記録',`${recordModeTabs('behavior',false)}${pupilDateNav(draft.date,'behavior-prev','behavior-next')}<section class="panel behavior-current"><div class="behavior-current-head"><div><span class="eyebrow">選択中のカテゴリー</span><h1 id="behavior-current-title" tabindex="-1">${esc(category.label)}</h1><p class="behavior-criterion">${esc(category.criterion)}</p></div><button type="button" class="button behavior-change-category" id="behavior-change-category">▶ カテゴリーを変更</button></div><div class="behavior-entry-toolbar"><p><strong>今日の○ <span id="behavior-today-count">${current.length}</span>人</strong><br><span class="muted small">児童名を押すと○。もう一度押すと解除します。</span></p>${teacherOrderControlHtml()}</div></section>${rosterHtml}<details class="behavior-examples behavior-examples-after"><summary>判断の具体例</summary><ul>${category.examples.map(item=>`<li>${esc(item)}</li>`).join('')}</ul></details>`);
