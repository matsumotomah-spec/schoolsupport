(function(){
  ...
  async function unzip(buffer){
    ...
    const total=u16(view,eocd+10),centralOffset=u32(view,eocd+16),entries=new Map();let cursor=centralOffset;
    for(let index=0;index<total;index++){
      if(u32(view,cursor)!==0x02014b50)throw new Error('Excelファイルの一覧を読み取れませんでした。');
      const method=u16(view,cursor+10),compressedSize=u32(view,cursor+20),nameLength=u16(view,cursor+28),extraLength=u16(view,cursor+30),commentLength=u16(view,cursor+32),localOffset=u32(view,cursor+42),name=decoder.decode(bytes.slice(cursor+46,cursor+46+nameLength));
