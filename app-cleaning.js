...
async function renderCleaningSetup(){
  ...
  const group=config.groups.find(g=>g.id===groupId),assigned=new Map(config.groups.flatMap(g=>g.memberIds.map(id=>[id,g.id])));
