"use strict";
  ...
  function syncGuideHtml(){return`<ol class="sync-steps"><li><b>1</b><span><strong>この端末の記録を保存</strong><small>記録を送る端末で「この端末の記録をファイルに保存」を押します。</small></span></li><li><b>2</b><span><strong>ファイルを別の端末へ渡す</strong><small>作成したJSONファイルをTeamsなどで別の端末から開ける場所へ置きます。</small></span></li><li><b>3</b><span><strong>別の端末で取り込む</strong><small>「別の端末で作ったファイルを選ぶ」から選び、内容を確認してまとめます。</small></span></li></ol><details class="sync-practice"><summary>本番前に、個人情報のないファイルで練習する</summary><p class="muted small">「練習用ファイル」をTeams経由で別の端末へ渡し、そこで選んでください。実際の名簿や記録には一切触れません。</p><div class="button-row"><button type="button" class="button" id="sync-practice-export">練習用ファイルを作る</button><label class="button">別の端末で練習用ファイルを選ぶ<input type="file" id="sync-practice-import" accept=".json,.json.txt,application/json,text/plain" hidden></label></div></details>`;}
  ...
  async function buildSyncPlan(payload){const stores=['years','classes','students','enrollments','records'];const plan={payload,writes:{},deletes:[],classDeletes:[],counts:{add:0,update:0,conflict:0,delete:0,skip:0}};const localTrash=await ClassDB.getAll('trash'),localRecords=await ClassDB.getAll('records');const deletedByRecord=new Map(localTrash.filter(item=>item.record?.id).map(item=>[item.record.id,item])),existingConflictFingerprints=new Set(localRecords.map(item=>item.conflictFingerprint).filter(Boolean));for(const store of stores){plan.writes[store]=[];for(const incoming of payload.data?.[store]||[]){if(store==='records'){const deleted=deletedByRecord.get(incoming.id);if(deleted&&String(deleted.deletedAt||'')>=String(incoming.updatedAt||'')){plan.counts.skip++;continue;}}const local=await ClassDB.get(store,incoming.id);if(!local){plan.writes[store].push(incoming);plan.counts.add++;continue;}if(newerThan(incoming,local)){plan.writes[store].push(incoming);plan.counts.update++;continue;}const conflict=store==='records'?syncConflictCopy(incoming,local,payload.device,()=>ClassDB.uid('syncConflict')):null;if(conflict&&!existingConflictFingerprints.has(conflict.conflictFingerprint)){plan.writes[store].push(conflict);existingConflictFingerprints.add(conflict.conflictFingerprint);plan.counts.conflict++;}else plan.counts.skip++;}}
    for(const incomingTrash of payload.data?.trash||[]){const recordId=incomingTrash.record?.id;if(!recordId)continue;const localRecord=await ClassDB.get('records',recordId),incomingRecord=plan.writes.records.find(item=>item.id===recordId),candidate=syncDeletionCandidate(incomingTrash,incomingRecord||localRecord,deletedByRecord.get(recordId));if(candidate){plan.deletes.push(candidate);plan.writes.records=plan.writes.records.filter(item=>item.id!==recordId);}else plan.counts.skip++;}
    for(const incomingTrash of payload.data?.trash||[]){if(incomingTrash.kind!=='classBundle')continue;const candidate=await syncClassDeletionCandidate(incomingTrash);if(candidate)plan.classDeletes.push(candidate);else plan.counts.skip++;}
    plan.deletes=[...new Map(plan.deletes.map(item=>[item.trash.record.id,item])).values()];plan.classDeletes=[...new Map(plan.classDeletes.map(item=>[item.trash.classBundle.class.id,item])).values()];plan.counts.delete=plan.deletes.length+plan.classDeletes.length;plan.meta=[];for(const incoming of payload.data?.meta||[]){if(!isYearSyncMeta(incoming.key)){plan.counts.skip++;continue;}const local=await ClassDB.get('meta',incoming.key);if(!local||newerThan(incoming,local)){plan.meta.push(incoming);local?plan.counts.update++:plan.counts.add++;}else plan.counts.skip++;}return plan;}
